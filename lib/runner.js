...
//
  ...
  run: function () {
    ...
    function describe(name, func) {
      this._sut = { name: name, specs: [] };
      _self.runnerData.contexts.push(this._sut);

      _self.initHooks(this);

      func();

      _after();

      if (_self.runnerData.options.verbose)
        sys.print("\n\n");
    }
    ...
    function it(desc, func) {
      ...
      _self.runnerData.total++;
      _sut.specs.push(spec);
      ...
      try {
        ...
        _self.runnerData.failCount++;
        _sut.hasFailures = true;
        _self.fail(e);
        spec.message = e;
      }
    ...
    coll.collect((this.runnerData.options.specdir || this.dir()), this.runnerData.options.filename,
        function (err, paths) {
          if(err) throw err;
          _self.readSpecs(paths, function(err, specs) {
            if(err) throw err;
            __execute__(specs);
          });
        });
