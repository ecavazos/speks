...
Runner.prototype = {
  ...
  run: function() {
    ...
    // will search the spec directory recursively for files
    // ending with "_spec.js" and add them to an array
    function collectSpecs(dir) {
      ...
      if(_self.opts.filename) {
        ...
        setTimeout(function() { promise.emitSuccess(); }, 100);
        return promise;
      }
      ...
      posix.readdir(dir).addCallback(function(files) {
        ...
        for(var i = 0, ii = files.length; i < ii; i++) {
          ...
          (function(filename) {
            ...
            var subPromise = subPromises[filename] = posix.stat(dir + filename);

            subPromise.addCallback(function(stat) {
              delete subPromise[filename];

              if(stat.isDirectory()) collectSpecs(dir + filename + '/');

              if(!filename.match(/_spec.js$/)) return;

              _files.push(dir + filename);
            });
    ...
    collectSpecs(_specDir).addCallback(function() {
      sys.puts(_files.length);
      execSpecs();
      outputResults();
    });
  }
